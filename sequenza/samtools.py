...
class bam_mpileup:
    ...
    def __init__(self, bam, fasta, q=20, Q=20, samtools_bin="samtools", regions=[]):
        ...
        self.proc2 = subprocess.Popen(
            samtools_pile, stdin=self.proc1.stdout, stdout=subprocess.PIPE, bufsize=4096
        )
...
def program_version(program):
    ...
    if program_bin is None:
        raise Exception(
            ("Please install %s or provide %s path in the " "appropriate argument")
            % (program, program)
        )
    else:
